...
internal class Operations
{
    ...
    /// <summary>
    /// Retrieves all records from the <c>PersonWithDuplicates</c> table in the database.
    /// </summary>
    /// <returns>
    /// A list of <see cref="PersonWithDuplicates"/> objects representing the current data 
    /// in the <c>PersonWithDuplicates</c> table.
    /// </returns>
    /// <remarks>
    /// This method uses Dapper to execute a SQL query that selects all rows from the 
    /// <c>PersonWithDuplicates</c> table, including their <c>Id</c>, <c>FirstName</c>, 
    /// <c>LastName</c>, and <c>BirthDay</c> fields.
    /// </remarks>
    ...
    /// <summary>
    /// Identifies and displays duplicate rows from a temporary table based on 
    /// specific columns (<c>Name</c> and <c>Email</c>).
    /// </summary>
    /// <remarks>
    /// This method retrieves data from a SQL query that populates a temporary table 
    /// with sample data, identifies duplicates based on the <c>Name</c> and <c>Email</c> 
    /// columns, and groups them. The results are then displayed in the console, 
    /// showing the duplicate entries and their associated IDs.
    /// </remarks>
    /// <example>
    /// Example output:
    /// <code>
    ...
    public void YourTableDuplicates()
    {
        ...
        List<ItemContainer> list = _cn.Query<ItemContainer>(SqlStatements.CreatePopulateTableGetDuplicates)
            .ToList();
        
        var result = list.GroupBy(x => new { x.Name, x.Email }, (key, group) => new
        {
            key.Name,
            key.Email,
            List = group.ToList()
